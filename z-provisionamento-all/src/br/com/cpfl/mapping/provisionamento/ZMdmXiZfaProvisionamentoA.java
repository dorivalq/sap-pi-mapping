...
/**
 * Implementação em Java do programa Abap Z_MDM_XI_ZFA_PROVISIONAMENTO_A
 * 
...
public class ZMdmXiZfaProvisionamentoA implements StreamTransformation {
	...
	private List<WaRfcOpt> TRfcOpt = new ArrayList<ZMdmXiZfaProvisionamentoA.WaRfcOpt>();
	...
	private List<TRfcFields> tRfcFields = new ArrayList<ZMdmXiZfaProvisionamentoA.TRfcFields>();
	...
	List<FsComfunction> tComfunction = new ArrayList<ZMdmXiZfaProvisionamentoA.FsComfunction>();
	...
	public void execute(InputStream inputStream, OutputStream outputStream) throws StreamTransformationException {
		try {
			...
			if (dateTimeE != null) {
				wDateTime = getValue((Element) dateTimeE.getParentNode(), "ami:value");
			...
			assignment.setAttribute("ident4", "add");
			...
			List<CpflZmodelObisc> listaModelo = consultarModelo(wModel);

			String wObiscode;
			for (CpflZmodelObisc model : listaModelo) {
				wObiscode = model.getId().getObiscode();
	...
	private Properties getProperties() {
		...
		props.put(Context.INITIAL_CONTEXT_FACTORY, PIMessages.getString("ejbclient.context"));
